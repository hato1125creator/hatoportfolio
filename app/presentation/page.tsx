    ...
    <main className="presentation-page">
      ...
      {slides.map((slide, index) => (
        <section className="presentation-slide" id={slide.id} key={slide.id}>
          ...
          {slide.image && <figure><img src={slide.image} alt="作品の実画面または当日運用の記録" /><figcaption>実際の画面・記録</figcaption></figure>}
          <a className="slide-next" href={index === slides.length - 1 ? "/" : `#${slides[index + 1].id}`}>{index === slides.length - 1 ? "ポートフォリオへ ↗" : "次へ ↓"}</a>
        </section>
      ))}
    </main>
  );
